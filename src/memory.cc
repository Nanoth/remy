...
void Memory::bit_flip(const int pos, const bool button)
{
    int the_pos = _current_num + pos;
    while(the_pos >= BitsetLength) the_pos -= BitsetLength;
    while(the_pos < 0) the_pos += BitsetLength; 
    _loss_rate -= _bs_loss_seq[the_pos];
    _loss_rate += button;
    _bs_loss_seq.set(the_pos,button);
}
...
{
  char tmp[ 256 ];
  snprintf( tmp, 256, "sewma=%f, rewma=%f, rttr=%f, lrate=%f, slowrewma=%f, rttd=%f, qdelay=%f", _rec_send_ewma, _rec_rec_ewma, _rtt_ratio, _loss_rate,  _slow_rec_rec_ewma, _rtt_diff, _queueing_delay );
  return tmp;
}
...
string Memory::str( unsigned int num ) const
{
  ...
  switch ( num ) {
    ...
    case 2:
      ...
      break;
    case 3:
      snprintf( tmp, 50, "lrate=%f ", _loss_rate );
      break;
    case 4:
